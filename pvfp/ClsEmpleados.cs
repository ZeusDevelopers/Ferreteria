...
namespace PVFP
{
    class ClsEmpleados
    {
        ...
        public DialogResult GuardarNuevoEmpleado(string usuario, string contra, int puesto, string nombre, string apellidos, string sexi,
        string direccion, string numero)
        {
            ...
            MySqlCommand _comando = new MySqlCommand(" select Count(Usuario) as d from sesion where usuario=@usu", conexion);
            _comando.Parameters.AddWithValue("@usu", usuario);
            MySqlDataReader _reader = _comando.ExecuteReader();
            int sesion_id = 0;
            while (_reader.Read())
            {
                sesion_id = Int32.Parse(_reader["d"].ToString());
            }
            _reader.Close();
            if (sesion_id == 0)
            {
                sesion_id = 0;
                _comando = new MySqlCommand("INSERT INTO Sesion VALUES(0,@Usuario,@Contra,@puesto) "
                   , conexion);
                ...
                _comando = new MySqlCommand("select Sesion_ID from sesion  " +
                    "order by Sesion_Id desc limit 1 ", conexion);
